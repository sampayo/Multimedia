class User < ActiveRecord::Base
  ...
  def self.seguidores(id)
    return Usuario.find_by_sql('select u.seudonimo, u.nombre from usuarios u, seguidores e where e.seguidor_id =u.id and e.usuarios_id = 1 order by e.simpatia Desc limit 5')
  end

  def self.siguiendo(id)
    return Usuario.find_by_sql('select u.seudonimo, u.nombre from usuarios u, seguidores e where e.usuarios_id =u.id and e.seguidor_id = 1 order by e.simpatia Desc limit 5')
  end
end
